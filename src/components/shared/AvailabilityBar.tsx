...
const AvailabilityBar: React.FC<Availability> = ({ label, percentage }) => {
  const displayPercentage = Number.isInteger(percentage) ? percentage : percentage.toFixed(2);
  ...
  return (
    <div className="text-center">
      ...
      <div className={`w-full ${barColor} rounded-full h-8 flex items-center justify-center mb-1`}>
        <span className={`text-lg font-bold ${textColor}`}>{displayPercentage}%</span>
      </div>
      <div>{icon}</div>
    </div>
  );
