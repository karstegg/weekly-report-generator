  ...
  return (
    <div className="bg-white shadow-md rounded-lg" style={slideStyle}>
      <main className="flex-grow p-6 pb-32 flex flex-col">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-x-6 gap-y-4 h-[350px]">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex flex-col"><div className="flex items-center mb-2"><CheckCircle className="text-green-600 mr-2" size={22} /><h3 className="text-xl font-bold text-green-800">Highlights</h3></div><ul className="text-base list-disc pl-5 space-y-1 ">{data.highlights.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex flex-col"><div className="flex items-center mb-2"><AlertTriangle className="text-red-600 mr-2" size={22} /><h3 className="text-xl font-bold text-red-800">Lowlights</h3></div><ul className="text-base list-disc pl-5 ">{data.lowlights.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex flex-col"><div className="flex items-center mb-2"><Clock className="text-yellow-600 mr-2" size={22} /><h3 className="text-xl font-bold text-yellow-800">Emerging Issues</h3></div><ul className="text-base list-disc pl-5 ">{data.emergingIssues.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex flex-col"><div className="flex items-center mb-2"><Wrench className="text-blue-600 mr-2" size={22} /><h3 className="text-xl font-bold text-blue-800">Priorities</h3></div><ul className="text-base list-disc pl-5 space-y-1 ">{data.priorities.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
        </div>
      </main>
